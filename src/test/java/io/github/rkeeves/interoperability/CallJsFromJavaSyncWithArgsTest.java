...
public class CallJsFromJavaSyncWithArgsTest {
     ...
     * We evaluate js with passed in args.
     *
     * Aka the function requires a DOM element so we pass in a WebElement (which hopefully resolves into an existing DOM Element).
     * Then we get back the value (Selenium wraps it into a function).
     *
    ...
    void test() {
        ...
        final var clickTarget = driver.findElement(By.cssSelector("#j_idt783 li[data-index='0']"));
        await.until(ExpectedConditions.attributeToBe(clickTarget, "aria-selected", "true"));
        JavascriptExecutor js = (JavascriptExecutor) driver;
        final var res = js.executeScript("return arguments[0].getAttribute('aria-selected')", clickTarget);
